...
The stub is what the registry scans; the `design-system` symlink is what keeps Open Design and agent usage on one source of truth. Do not copy the whole package into `custom/` as a second editable source.
